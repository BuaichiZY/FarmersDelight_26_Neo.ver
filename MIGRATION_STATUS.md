...
## Fix3 wild-crop regression

Both the pre-fix resources and survival checks used `minecraft:dirt`, which
already excludes grass and podzol in this Minecraft version. A runtime test
reproduced zero wild carrots before the fix. The port now uses
`minecraft:substrate_overworld` for land-crop placement, tomato terrain and
wild-crop/rice survival, including mud beneath shallow water.

The new GameTest verifies biome injection and the complete placement pipeline
of seven wild crops and both mushroom colonies on controlled terrain, including
rarity, biome, heightmap and survival checks. All six required tests pass on the
declared minimum NeoForge version. This is not a survey of naturally explored
terrain. Biome restrictions and generation frequency are unchanged; existing
chunks are not retro-populated, and default superflat without decoration is
...
- `build` succeeds and produces both
  `build/libs/FarmersDelight-26.2-1.3.3-fix3.jar` and
  `build/libs/FarmersDelight-26.2-1.3.3-fix3_source.jar`.
...
- Custom food-effect components store the registry's real effect holder instead
  of NeoForge's deferred wrapper, allowing CraftBukkit-based hybrid servers to
  translate Nourishment potion-effect events without disconnecting the player.
