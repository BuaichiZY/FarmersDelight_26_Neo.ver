...
public class SmokerCookFunction extends LootItemConditionalFunction
{
	...
	protected SmokerCookFunction(Optional<Holder<LootItemCondition>> condition) {
		super(condition);
	}
	...
	protected ItemStack run(ItemStack stack, LootContext context) {
		...
		Optional<SmokingRecipe> recipe = context.getLevel().getServer().getRecipeManager().getRecipes().stream()
				.map(RecipeHolder::value)
				.filter(SmokingRecipe.class::isInstance)
				.map(SmokingRecipe.class::cast)
				.filter(r -> r.input().test(stack))
				.findFirst();
		if (recipe.isPresent()) {
			ItemStack resultStack = recipe.get().assemble(new SingleRecipeInput(stack));
