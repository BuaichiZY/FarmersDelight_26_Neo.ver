...
public class CookingPotMenu extends RecipeBookMenu
{
	...
	public CookingPotMenu(final int windowId, final Inventory playerInventory, final RegistryFriendlyByteBuf data) {
	...
	private static CookingPotBlockEntity getBlockEntity(final Inventory playerInventory, final RegistryFriendlyByteBuf data) {
	...
	public PostPlaceAction handlePlacement(boolean useMaxItems, boolean allowDroppingItemsToClear, RecipeHolder<?> recipe,
			ServerLevel serverLevel, Inventory playerInventory) {
		...
		return ServerPlaceRecipe.placeRecipe(new ServerPlaceRecipe.CraftingMenuAccess<CookingPotRecipe>() {
			...
			@Override
			public boolean recipeMatches(RecipeHolder<CookingPotRecipe> cookingRecipe) {
				return CookingPotMenu.this.recipeMatches(cookingRecipe);
			}
		}, 3, 2, inputSlots, inputSlots, playerInventory, (RecipeHolder<CookingPotRecipe>) recipe,
				useMaxItems, allowDroppingItemsToClear);
	}
