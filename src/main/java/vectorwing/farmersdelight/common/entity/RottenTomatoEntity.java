...
public class RottenTomatoEntity extends ThrowableItemProjectile
{
		...
		if (id == 3) {
			...
			for (int i = 0; i < 12; ++i) {
				this.level().addParticle(iparticledata, this.getX(), this.getY(), this.getZ(),
						((double) this.getRandom().nextFloat() * 2.0D - 1.0D) * 0.1F,
						((double) this.getRandom().nextFloat() * 2.0D - 1.0D) * 0.1F + 0.1F,
						((double) this.getRandom().nextFloat() * 2.0D - 1.0D) * 0.1F);
			}
		}
	}
	...
	protected void onHitEntity(EntityHitResult result) {
		...
		this.playSound(ModSounds.ENTITY_ROTTEN_TOMATO_HIT.get(), 1.0F, (this.getRandom().nextFloat() - this.getRandom().nextFloat()) * 0.2F + 1.0F);
	}
